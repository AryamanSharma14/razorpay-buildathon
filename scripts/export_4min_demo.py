...
def main():
    ...
    assets_dir = root_dir / "build" / "demo_assets_4min"
    raw_video_dir = root_dir / "build" / "raw_video_4min"
    master_audio = assets_dir / "demo_4min_master_audio.mp3"
    build_mp4 = root_dir / "build" / "demo_video_4min.mp4"
    root_mp4 = root_dir / "demo_video_4min.mp4"
    main_demo_mp4 = root_dir / "demo_video.mp4"
    ...
    if not master_audio.exists():
        print("[2/5] Master audio not found, generating now...")
        subprocess.run([sys.executable, str(root_dir / "scripts" / "generate_voiceovers_4min.py")], check=True, cwd=root_dir)
    
    audio_info = MP3(master_audio)
    audio_duration = audio_info.info.length
    print(f"[2/5] Master audio verified: {audio_duration:.2f}s ({audio_duration/60:.2f} min) - {master_audio.name}")
    ...
    print("[4/5] Running Playwright browser recording script (1920x1080, ~4 minutes)...")
    recorder_script = root_dir / "scripts" / "record_4min_demo.cjs"
    ...
    shutil.copy2(build_mp4, main_demo_mp4)
    file_size_mb = root_mp4.stat().st_size / (1024 * 1024)

    print("\n" + "=" * 60)
    print("4-MINUTE MASTER DEMO VIDEO EXPORT COMPLETE!")
    print(f"Output file: {root_mp4}")
    print(f"Main copy:   {main_demo_mp4}")
